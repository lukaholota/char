...
export async function generateStaticParams() {
  return getAllBackgrounds("RULES_2024").flatMap((b) => [
    { backgroundId: b.slug },
...
export async function generateMetadata({
  ...
  const title = `${background.name} [${background.engName}] — походження D&D 2024`;
  const description = getDescriptionSnippet(
    `${background.name} (${background.engName}) — походження D&D 5e (PHB 2024). ${background.shortDescription || background.description}`
  );
  const url = `${process.env.NEXT_PUBLIC_SITE_URL || "https://char.holota.family"}/2024/backgrounds/${background.slug}`;
...
export default async function Background2024DetailPage({
  params,
}: {
  params: Promise<{ backgroundId: string }>;
}) {
      ...
      <div className="sticky top-0 z-20 border-b border-white/10 bg-slate-950/20 backdrop-blur-xl">
        <div className="mx-auto max-w-2xl px-4 py-3">
          ...
          >
            ...
            Назад до походжень 2024
          </Link>
        </div>
      </div>

      <div className="mx-auto max-w-2xl px-4 py-6">
        <BackgroundDetailCard background={background} is2024 />
      </div>
    </div>
  );
}
