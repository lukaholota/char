...
export async function generateMetadata({
  ...
  const masteryPart = weapon.masteryNameUa ? ` | Майстерність: ${weapon.masteryNameUa}` : "";
  const title = `${weapon.nameUa} [${weapon.engName}] — ${typeLabel} (2024)`;
  const description = getDescriptionSnippet(
    `${weapon.nameUa} (${weapon.engName}) D&D 2024 — ${typeLabel}. Шкода: ${weapon.damage || "-"} ${damageTypeLabel}.${masteryPart}`
  );
  const url = `${process.env.NEXT_PUBLIC_SITE_URL || "https://char.holota.family"}/2024/weapons/${toEntitySlug(weapon.engName)}`;
...
export default async function Weapon2024DetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <div className="w-full min-h-screen bg-[radial-gradient(circle_at_50%_0%,rgba(192,74,224,0.05),transparent_50%)] overflow-x-hidden">
      ...
      <div className="mx-auto max-w-2xl px-4 py-6">
        <WeaponDetailCard weapon={weapon} is2024={true} />
      </div>
    </div>
  );
}
