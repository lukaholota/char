...
export async function generateStaticParams() {
  return getAllMetamagic("RULES_2024").map((option) => ({ slug: toEntitySlug(option.engName) }));
}
...
export async function generateMetadata({
  ...
  const metamagic = getMetamagicByIdOrSlug(slug, "RULES_2024");
  ...
  const title = `${metamagic.nameUa} [${metamagic.engName}] — Метамагія (2024)`;
  const description = getDescriptionSnippet(
    `${metamagic.nameUa} (${metamagic.engName}) — метамагія чародія (2024), ${describeMetamagicCost(metamagic).toLowerCase()}. ${metamagic.shortDescription || metamagic.description}`
  );
  const url = `${process.env.NEXT_PUBLIC_SITE_URL || "https://char.holota.family"}/2024/metamagic/${toEntitySlug(metamagic.engName)}`;
...
export default async function Metamagic2024DetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
      ...
      <div className="sticky top-0 z-20 border-b border-white/10 bg-slate-950/20 backdrop-blur-xl">
        <div className="mx-auto max-w-2xl px-4 py-3">
          ...
          >
            ...
            Назад до метамагії (2024)
          </Link>
        </div>
      </div>

      <div className="mx-auto max-w-2xl px-4 py-6">
        <MetamagicDetailCard metamagic={metamagic} is2024={true} />
      </div>
    </div>
  );
}
