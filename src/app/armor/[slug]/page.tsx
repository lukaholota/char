...
export async function generateMetadata({
  ...
  const armor = getArmorByIdOrSlug(slug, "RULES_2014");
  ...
  const title = `${armor.nameUa} [${armor.engName}] — ${typeLabel}`;
  const description = getDescriptionSnippet(
    `${armor.nameUa} (${armor.engName}) — ${typeLabel}. Базовий КБ: ${armor.baseAC}. Вага: ${armor.weight}, вартість: ${armor.cost}.`
  );
  const url = `${process.env.NEXT_PUBLIC_SITE_URL || "https://char.holota.family"}/armor/${toEntitySlug(armor.engName)}`;
...
export default async function ArmorDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_50%_0%,rgba(45,212,191,0.05),transparent_50%)] overflow-x-hidden">
      ...
      <div className="mx-auto max-w-2xl px-4 py-6">
        <ArmorDetailCard armor={armor} is2024={false} />
      </div>
    </div>
  );
}
