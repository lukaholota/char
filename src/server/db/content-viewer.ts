...
export type ContentViewer = { userId: number | null; isModerator: boolean };
...
export async function findContentViewer(): Promise<ContentViewer> {
  ...
  if (!email) return { userId: null, isModerator: false };
  const user = await prisma.user.findUnique({ where: { email }, select: { id: true } });
  return { userId: user?.id ?? null, isModerator: Boolean(user) && isModeratorEmail(email) };
}
