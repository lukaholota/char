...
import { armorTypeTranslations, armorTranslations } from "@/lib/refs/translation";
import { Shield, Settings2, AlertTriangle, CheckCircle2 } from "lucide-react";
...
type CombatSlideProps = {
  ...
  isReadOnly?: boolean;
  /// Слайд свідомо отримує власний лист, а не синтетичний: обладунок, зброя й предмети у формі
  /// не діють, і показувати їх із характеристиками звіра означало б брехати про спорядження.
  wildshape: WildshapeState;
};

const CombatSlide = memo(function CombatSlide({ pers, onPersUpdate: _onPersUpdate, isReadOnly, wildshape }: CombatSlideProps) {
// Redundant stats removed
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Дика форма — сама себе ховає, якщо персонаж її не має */}
      <WildshapeCard
        persId={pers.persId}
        persName={pers.name}
      ...
      {wildshape.active && (
        <p className="rounded-lg border border-amber-400/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-200/90">
          У звіриній формі спорядження не діє — воно злилося з новою подобою. Числа нижче
          лишаються вашими; атаки й дії звіра дивіться у статблоці.
        </p>
      )}

      <WeaponsCard pers={pers} isReadOnly={isReadOnly} isPending={isPending} onCustomize={setSelectedWeapon} />

      <WeaponMasteryCard pers={pers} isReadOnly={isReadOnly} />
