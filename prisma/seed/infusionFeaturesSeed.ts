...
export const seedInfusionFeatures = async ( prisma: PrismaClient ) => {
	...
	const features: SeedFeatureCreateInput[] = [
		...
		},
		{
			...
			description: 'Цей предмет може посилати удар носієві, щоб перекопцентрувати його розум. Предмет має 4 заряди. Коли носій не успішно проходить рятувальний кидок Статури, щоб підтримати концентрацію на закляттях, носій може використати свою реакцію, щоб витратити 1 заряд предмета, щоб замість цього успішно пройти. Предмет отримує 1к4 витрачених зарядів щодня на світанку.',
