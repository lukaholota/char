 ...
 * `data/2024/normalized/classes.json`. Ідемпотентний — повторний запуск дає ті самі рядки.
 *
 *   bun run seed:class-spells-2024:test   → .env.test, клон spells_test
 *   bun run seed:class-spells-2024:prod   → .env, робоча база (запускає власник)
 */
...
import { seedClassSpells2024 } from "../prisma/seed/classSpells2024";
  ...
  if (name !== "test" && name !== "prod") {
    throw new Error(
      "Сідер пише в базу, тому цільову базу треба назвати явно — дефолту немає.\n" +
        "  bun run seed:class-spells-2024:test   → .env.test, клон spells_test\n" +
        "  bun run seed:class-spells-2024:prod   → .env, робоча база spells",
    );
  }
...
async function main() {
  ...
  console.log(`🚀 KR31.5 — завжди підготовлені заклинання класів 2024 для бази "${dbName}" (--target ${target})…\n`);

  await seedClassSpells2024(prisma);
