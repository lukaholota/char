/**
 * Описи підкласів 2024 — переливає `data/2024/normalized/subclasses.json` у базу.
 *
 * Потрібен, коли правка джерела має доїхати до каталогів без повного `seed:2024`: сідер
 * підкласів робить лише `upsert` підкласу та його рис, рядків не створює й не видаляє.
 *
 *   bun run seed:subclasses-2024:test   → .env.test, локальна spells_test
 *   bun run seed:subclasses-2024:prod   → .env, робоча база
 */
  ...
  if (name !== "test" && name !== "prod") {
    throw new Error(
      "Сідер пише в базу, тому цільову базу треба назвати явно — дефолту немає.\n" +
        "  bun run seed:subclasses-2024:test   → .env.test, локальна spells_test\n" +
        "  bun run seed:subclasses-2024:prod   → .env, робоча база spells",
    );
  }
...
async function main() {
  ...
  console.log(`🚀 Описи підкласів 2024 для бази "${dbName}" (--target ${target})…\n`);
