...
import { seedRadiantTerminology } from "../prisma/seed/radiantTerminology";
...
function readTargetName(argv: string[]): TargetName {
  ...
  throw new Error(
    "Сідер пише в базу, тому цільову базу треба назвати явно — дефолту немає.\n" +
      "  bun run seed:radiant:test   → .env.test, клон spells_test\n" +
      "  bun run seed:radiant:prod   → .env, робоча база spells"
  );
}
...
async function main(): Promise<void> {
  ...
  try {
    console.log(`🔮 Зведення radiant у базі "${databaseName}" (--target ${target})…\n`);
    const applied = await seedRadiantTerminology(prisma);
    console.log(`\n✅ Термінологія radiant: застосовано ${applied.length} записів.`);
  } finally {
