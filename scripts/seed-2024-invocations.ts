/**
 * KR18.8 — потойбічні виклики 2024: механіка Чорнокнижника (Feature + ChoiceOption +
 * ClassChoiceOption на WARLOCK_2024).
 *
 * Ганяється поверх `seed:2024`: клас WARLOCK_2024 уже має бути в базі. Ідемпотентний — усе на
 * upsert, повторний запуск нічого не дублює.
 *
 *   bun run seed:invocations-2024:test   → .env.test, клон spells_test
 *   bun run seed:invocations-2024:prod   → .env, робоча база (запускає власник)
 */
  ...
  if (name !== "test" && name !== "prod") {
    throw new Error(
      "Сідер пише в базу, тому цільову базу треба назвати явно — дефолту немає.\n" +
        "  bun run seed:invocations-2024:test   → .env.test, клон spells_test\n" +
        "  bun run seed:invocations-2024:prod   → .env, робоча база spells",
    );
  }
...
async function main() {
  ...
  console.log(`🚀 KR18.8 — потойбічні виклики 2024 для бази "${dbName}" (--target ${target})…\n`);

  await seedInvocations2024(prisma);
