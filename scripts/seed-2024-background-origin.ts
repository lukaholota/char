 ...
 *
 * Ганяється поверх `seed:2024`: самі походження вже в базі, тут переписуються два поля —
 * `tool_proficiencies` і `items` — з нормалізованих даних. Ідемпотентний: чистий update,
 * повторний запуск дає той самий результат.
 *
 *   bun run seed:background-origin-2024:test   → .env.test, клон spells_test
 *   bun run seed:background-origin-2024:prod   → .env, робоча база (застосовує власник)
 */
...
import { seedBackgroundOrigin2024 } from "../prisma/seed/backgroundOrigin2024";
  ...
  if (name !== "test" && name !== "prod") {
    throw new Error(
      "Сідер пише в базу, тому цільову базу треба назвати явно — дефолту немає.\n" +
        "  bun run seed:background-origin-2024:test   → .env.test, клон spells_test\n" +
        "  bun run seed:background-origin-2024:prod   → .env, робоча база spells",
    );
  }
...
async function main() {
  ...
  console.log(`🚀 KR18.7 — майно й інструменти походжень 2024 для бази "${dbName}" (--target ${target})…\n`);

  await seedBackgroundOrigin2024(prisma);
