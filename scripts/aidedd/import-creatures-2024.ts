...
export const TRANSLATIONS_DIR = join(AIDEDD_DIR, "translations/monsters-2024");
...
export function buildImportedCreatures2024(): GeneratedCreature[] {
  const ids = readManifestIds();

  return readTranslationBatches().map((translation) => {
    const creatureId = ids.get(translation.slug);
    if (!creatureId) {
      throw new Error(`У маніфесті немає рядка для «${translation.slug}»`);
    }
...
function readManifestIds(): Map<string, number> {
  ...
  return new Map(
    rows.filter((row) => row.edition === "RULES_2024").map((row) => [row.slug, row.creatureId])
  );
}

function readParsedCreature(slug: string) {
  const path = join(findRawDir("monsters-2024"), `${slug}.html`);
  if (!existsSync(path)) {
    throw new Error(
      `Немає кешованої сторінки ${path} — запусти bunx tsx scripts/aidedd/fetch-pages.ts --catalog monsters-2024`
    );
  }
  return parseMonster2024(readFileSync(path, "utf-8"), slug);
}
