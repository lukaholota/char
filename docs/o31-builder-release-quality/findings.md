...
**2026-09-20, прохід по гейтах кроків:** крок навичок замикав конструктор намертво в обох редакціях — розбір і решта перевірених кроків у [audit/G20-step-gates-2026-09-20.md](audit/G20-step-gates-2026-09-20.md). Сім знахідок, усі закриті того ж дня: крок навичок не перераховував гейт; фіксовані расові навички рахувались як вибори; вимога могла перевищити те, що є на екрані; левелап пускав далі з невитраченими навичками; расам підставляли всі 18 навичок замість їхнього переліку; крок експертизи читав навички повз частину джерел; третій вибір на кроці опцій риси стирав підпис «Вже володієте цією навичкою».
...
**Статус:** ✅ закрито раніше в [KR31.3](kr31.3-feature-resources-2024.md) (класові лічильники, 2026-09-06); звірено 2026-09-13 (KR31.5). Усі сім класових фіч із безкоштовним застосуванням мають лічильник у `classes.json` і в обох базах: Favored Enemy — LONG_REST 2/3/4/5/6 за рівнями 1/5/9/13/17; Paladin's Smite, Faithful Steed, Divine Intervention, Contact Patron, Mystic Arcanum — 1 на довгий; Signature Spells — 2 на короткий. Spell Mastery лишається без лічильника, бо вона безлімітна. Тримає `tests/content/class-feature-uses-2024.test.ts`. Доказ у знахідці знято до сіду KR31.3.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.5). Рядок слотів картки класу рахує чиста `findClassTableSpellSlots` (`src/rules/class-table-spell-slots.ts`); для напівзаклинача 2024 — через `getMaximumStandardSpellSlots`, тобто тим самим шляхом, що й персонаж. Гілки 2014 (статична HALF, костур артифіцера) перенесено без змін. `tests/rules/class-table-spell-slots.test.ts` звіряє всі 20 рівнів паладина й слідопита з таблицями `data/2024/srd/classes.md`. Живий Chromium: паладин 2024, рівень 1 — 2 слоти, рівень 5 — 4/2.
...
**Статус:** ✅ закрито 2026-09-14 у KR31.5 ([Р43](../DECISIONS.md#р43)): вибір є і при створенні, і на підвищенні рівня. Книга чарівника — 6 на 1-му рівні й +2 за рівень, рядками з `isPrepared = false`; підготовлені лише з книги, зокрема з уже записаного. 2026-09-14 лист під рядком чарівника 2024 показує «Книга заклинань: X / Y» (`tallyWizardSpellbook`, Y = 6 + 2 × (рівень − 1)), жовтим, поки книга неповна. Тести: `tests/rules/class-spell-choices-2024.test.ts` (+1), `tests/components/magic-slide-prepared-limits.test.tsx` (+1), обидва доведено зламом.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.11). Каталоги в git: 20 із 21 файлу `src/lib/generated/` відстежуються. Новий `rule-term-cards.json` (O34, паралельна сесія) ще `??` — його має закомітити та сама робота, що його імпортує. Збірку з чистого клону можна перевірити лише після коміту робочого дерева.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.11). Виміряно скриптом на `pg` (читання): 17 маркерів — сіди 2024 (класи, заклинання, риси, 803 фічі з викликами, 445 предметів, 46 опцій видів, заклинання опцій, риси з рівнем), `DRACONIC_RESILIENCE`, 16 обладунків 2024, KR27.5, апостроф (0), тексти фіч 2014 (0 порожніх), `weapon_mastery_progression`, `subclass_spell` — у робочій базі дорівнюють `spells_test`. Дій над робочою базою перед релізом не лишилося; каталоги перегенеровані, але ще не закомічені.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.11) — артефакт аудиторської копії дерева, не дефект: `src/rules/ability-score-ceiling.ts` у репозиторії є.
...
**Статус:** 🔴 передано в KR31.15 (дика форма) 2026-09-13: тест друїда 2024 через базу пишеться разом із лічильником використань.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.11). Причина — не код, а вимір: планка рахувала `src/rules/` лише тестами `tests/rules/coverage/`. `vitest.rules.config.mts` тепер ганяє всі тести правил без бази (`src/rules/**`, `tests/rules/**`, `tests/rules-2024/**`, мінус спільний `vitest.db-integration-files.mts`): 89 % інструкцій, 83 % гілок, 87 % функцій, 91 % рядків — планка 80 % без змін. Джоба отримала `prisma generate`, скрипт — мертву адресу бази.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.11) — рішення власника: межі заморожено на поточних розмірах (`config/ui-decomposition-guard.json`, 15 файлів: 11 підрослих і 4 нові). Сторож і далі не дає їм рости; розбиття великих компонентів — у KR31.14.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.11). `scripts/smoke.sh` перевіряє ще `/char/create`, `/2024`, `/2024/char`, `/2024/spells`, `/2024/classes`. `tests/routes/smoke-script.test.ts` ганяє сам скрипт проти локального сервера, що віддає 500 на одному маршруті; зі старим списком червоніли три випадки.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.10). `tests/pdf/character-pdf-fidelity.test.ts` будує персонажа 2024, генерує PDF і читає поля з віджетів кожної сторінки; знімок імен заповнених полів першої сторінки й листа заклинань червоніє на перейменованому полі (перевірено).
...
**Статус:** ✅ закрито 2026-09-13 (KR31.11). Мертвий `useSession()` прибрано з `EditionSwitcher`; запис у `docs/KNOWN-BUGS.md` позначено виправленим.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.11). `spell-add-to-pers-2024` зелений. `bun run test:no-db` на 2026-09-13 — 2628 із 2632: чотири падіння в `radiant-term-sweep` і `rule-term-mentions-linked` належать незавершеній роботі O34 / зачистки «променист-» і мають позеленіти з її комітом.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.11). `src/app/sitemap.ts` збирає обидві редакції одним `collectEditionPaths`: 3336 → 4421 адрес, жодна адреса 2014 не загубилась. `tests/routes/sitemap-editions.test.ts` (червоний до правки). Перемикач редакції лишився кнопкою — sitemap покриває краулер.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.11). Застарілі коментарі про тунель і живу базу в джобі `image` прибрано. `.github/actions/db-tunnel/` лишено: у самому файлі записано, що це свідомо. `network=host` позначено як залишок — прибирати разом із перевіреним прогоном образу.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.11). `tests/routes/legacy-creator-redirect.test.ts`: `/char` у `matcher` і 308 на `/char/create`; без рядка в `matcher` червоніє.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.8). Запит `armor` у `character-creation.ts` фільтрує `ruleset` персонажа. Golden створення тепер пише редакцію кожного рядка обладунку й зброї (`tests/helpers/normalize-golden.ts`), тож `monk-unarmored-defense` / `barbarian-unarmored-defense` / `simple-asi-system` без фільтра червоніють на `:RULES_2024`. У робочій базі 2026-09-13 — 21 монах і 16 варварів 2014 із рядком 2024 (створені 2026-09-08…12, з сіду KR27.8); КЗ не мінявся, бо формула однакова. Виправлення даних — `db/changes/2026-09-13-kr31.8-unarmored-defense-2014-rows.sql`, застосовує власник.
...
**Статус:** ✅ закрито раніше в KR27.8 — `grantAlternativeArmorClassFormulas` (`src/server/db/armor-class-formulas.ts`) видає рядки 2024 за наданими фічами й викликається і створенням, і підвищенням рівня. Звірено 2026-09-13 (KR31.8).
...
**Статус:** ✅ закрито 2026-09-13 (KR31.8). `getBaseEquipment(ruleset)` без константи `ACTIVE_RULESET`; `AddWeaponDialog` і `AddArmorDialog` беруть `pers.ruleset`. `addWeapon` / `addArmor` відмовляють, коли переданий рядок каталогу іншої редакції (серверні заглушки власної зброї й `HOMEBREW`-обладунку не зачеплено — рядка 2024 для них немає). `tests/db/sheet-equipment-ruleset.test.ts`: каталог 2024 — лише 2024 і Довгий меч із SAP; Довгий меч і Кольчуга 2014 персонажу 2024 — відмова. Тест KR6.3 у `ruleset-server-filter.test.ts` переписано на явну редакцію. У робочій базі персонажів із чужою зброєю — 0.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.8). `getUserPersesMagicItemIndex(ruleset)` віддає лише персонажів редакції сторінки каталогу; `toggleMagicItemForPers` не додає предмет чужої редакції (`isMagicItemOfPersRuleset`), але прибрати вже доданий дозволяє. Той самий гейт — для заклинань: `getUserPersesSpellIndex(ruleset)` і `setSpellPresenceForPersByLink`; тест KR25.2, що додавав заклинання 2024 персонажу 2014, переведено на персонажа 2024. `tests/actions/magic-item-actions.test.ts`, `tests/actions/spell-actions.test.ts`. У робочій базі чужих предметів і заклинань — 0.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.8). `AddMagicItemDialog` приймає `ruleset` і відкриває `/2024/magic-items` для персонажа 2024; ідентифікатори каталогу 2024 звірено з `magic_item` (445 із 445). `tests/components/add-magic-item-dialog.test.tsx`.
...
**Статус:** ✅ закрито 2026-09-14 (KR31.5). Діалог уже бере каталог за редакцією персонажа, бо
корінь `P2-elf-wizard-01` виправлено 2026-09-04. Лишався наслідок, який назвав скептик:
`setSpellPresenceForPers` приймав номер заклинання будь-якої редакції. Тепер сервер відмовляє
«Це заклинання належить іншій редакції правил»; гейт — `tests/actions/spell-actions.test.ts`.
...
**Статус:** ⛔ не робиться — прийнята архітектура: рішення власника 2026-08-28 (`docs/o21-user-signals/questions.md`, питання 4, варіант А) і KR18.8 «суфікс, не складений ключ». Жива шкода подана окремими знахідками; перевірено 2026-09-13 (KR31.8).
...
**Статус:** ✅ закрито 2026-09-13 (KR31.8). Порівняння риси за назвою безпечне, поки персонаж тримає риси лише своєї редакції, — тепер це гарантує сервер: `addFeatToPers` відмовляє рисі чужої редакції (`isFeatOfPersRuleset`). Усі інші пошуки риси йдуть за `featId` або з фільтром редакції. `tests/actions/feat-actions-ruleset.test.ts`: «Пильний» 2024 персонажу 2014 — відмова. У робочій базі один такий запис (персонаж 13808, «Пильний» 2024, pers_feat 7191) — рішення власника 2026-09-13: замінити на 2014, `db/changes/2026-09-13-kr31.8-alert-2014-for-pers-13808.sql`.
...
**Статус:** 🔴 частково (KR31.8, 2026-09-13). `getSpellsList` видалено разом із тестом, що її пінив, і константою `ACTIVE_RULESET` у `spell-actions.ts`. `loadFightingStyleOptions` лишається: вона живе на мертвому другому майстрі підвищення й піде разом із ним (`L08-levelup-machine-12`). Гейт `tests/content/server-db-fixed-ruleset.test.ts` не пускає нову константу редакції в `src/server/db` без запису причини.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.7). Одна `clonePersWithRelations` (`src/lib/logic/pers-duplication.ts`) із типізованою ціллю копії (`PersCloneTarget`) обслуговує копію персонажа, копію теки, копію теки за посиланням, копію за посиланням і знімок рівня. `tests/db/pers-copy-completeness.test.ts`: складний персонаж 2024 зі всіма звʼязками копіюється кожним шляхом, граф порівнюється поле-в-поле за `schema.prisma`; до правки червоніли всі пʼять шляхів. `PERS_DUPLICATION_INCLUDE` несе `resourcePools`, `wildshapes` і `bastion` із приміщеннями й ходами.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.7). `copyPersByToken` — виклик тієї самої `clonePersWithRelations`; до правки копія за посиланням губила ще й `ruleset`, тобто персонаж 2024 ставав 2014. Одна `clonePersWithRelations` (`src/lib/logic/pers-duplication.ts`) із типізованою ціллю копії (`PersCloneTarget`) обслуговує копію персонажа, копію теки, копію теки за посиланням, копію за посиланням і знімок рівня. `tests/db/pers-copy-completeness.test.ts`: складний персонаж 2024 зі всіма звʼязками копіюється кожним шляхом, граф порівнюється поле-в-поле за `schema.prisma`; до правки червоніли всі пʼять шляхів.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.7). Include листа винесено в `PERS_SHEET_INCLUDE` (`src/server/db/pers-sheet-include.ts`); `getPersById` додає лише `user`, `getPersByShareToken` бере той самий include в обох гілках. `tests/db/share-sheet-parity.test.ts`: спільний персонаж дорівнює власному без `user`; до правки бракувало `pers_weapon_mastery` і `classOptionalFeatures`.
...
**Статус:** ✅ закрито раніше в KR31.12 (BUG-010); для 2024 — `tests/db/long-rest-spell-slots-2024.test.ts` (KR31.6). Звірено 2026-09-13.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.7) разом із `L12-secondary-flows-03`.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.7). Знімок більше не окрема реалізація копії; активація — рішення власника 2026-09-13 «лише як копія»: знімок можна переглянути й скопіювати в нового персонажа, відновлення на місці не робиться. Мертву `activateSnapshot` (жодного виклику з UI) прибрано разом із кроком тесту, що її пінив.
...
**Статус:** ⛔ не робиться (KR31.7) — рішення власника 2026-09-13 «лише як копія»: знімок можна переглянути й скопіювати в нового персонажа, відновлення на місці не робиться. Мертву `activateSnapshot` (жодного виклику з UI) прибрано разом із кроком тесту, що її пінив.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.7). Знімок рівня — `clonePersWithRelations` з `snapshotOf`, тож переносить майстерність, ресурси, дику форму, бастіон, інфузії, налаштування зброї й стан магічних предметів. Одна `clonePersWithRelations` (`src/lib/logic/pers-duplication.ts`) із типізованою ціллю копії (`PersCloneTarget`) обслуговує копію персонажа, копію теки, копію теки за посиланням, копію за посиланням і знімок рівня. `tests/db/pers-copy-completeness.test.ts`: складний персонаж 2024 зі всіма звʼязками копіюється кожним шляхом, граф порівнюється поле-в-поле за `schema.prisma`; до правки червоніли всі пʼять шляхів.
...
**Статус:** ⛔ не робиться — рішення власника 2026-09-13: експорт і імпорт JSON не потрібні (резервні копії робить база, передавання закриває копія за посиланням).
...
**Статус:** ✅ закрито 2026-09-13 (KR31.7). `getFolderByShareToken` віддає `ruleset`, підклас і мультикласи; підпис збирає `collectPersClassNames` / `collectPersSubclassNames` (`src/lib/logic/pers-class-names.ts`) — ті самі функції тепер у двох домашніх сторінках і в `duplicatePers`, де було три копії. `tests/db/share-sheet-parity.test.ts`, `tests/logic/pers-class-names.test.ts`.
...
**Статус:** ✅ закрито раніше в робочому дереві (незакомічена правка іншої сесії), звірено 2026-09-14: `data/2024/normalized/backgrounds.json`, `translation.ts` і дзеркало дають ті самі 16 назв без суфікса.
...
**Статус:** ✅ закрито 2026-09-14 у KR31.14 без DDL: `CARPENTERS_TOOLS` в enum уже був. Фермеру в обох файлах-джерелах поставлено `CARPENTERS_TOOLS`, `seed:2024 --only backgrounds,backgrounds-existing` прогнано на клоні, а `--only backgrounds-existing` — у проді після звірки всіх 16 рядків із файлами (відрізнялося лише це поле). `creator-content-2024.json` перегенеровано з проду — змінився тільки цей масив і порядок походжень.
...
**Статус:** ✅ закрито раніше, звірено 2026-09-14: `backgroundsData.ts` віддає `equipmentPackage` українською, англійського рядка в коді немає.
...
**Статус:** ✅ закрито 2026-09-14 (KR31.5). «Слоти заклинань» у заголовку листа, підказках, Магії
пакту, повідомленнях відпочинку й помилках `spell-slots.ts`. «Складість Ряткидка» → «Складність
ряткидка». Абревіатуру лишено «СК»: `dictionary.json` → `generalTerms.DC` = «СК», а словник виграє в
аудиту. Гейт — `tests/content/spell-slot-term.test.ts`.
...
**Статус:** ✅ закрито 2026-09-14 у KR31.14 на показі, а не в даних: картка зброї пише `formatDiceUkr(calculateWeaponDamageDice(…))`, як уже робив PDF. Так «к» отримують і кубик Бойових мистецтв, і вписаний гравцем «2d4». Парсери кидка (`WeaponsCard`, `martial-arts.ts`) приймають обидва записи, тож дані не чіпались.
...
**Статус:** ✅ закрито 2026-09-14 рішенням власника: «Показник небезпеки», скорочення «ПН». Словник (`challengeRating`, `rules2024.statblockFields.cr`), картка Дикої форми й рядки обмежень (`wildshape.ts`), статблок і фільтр бестіарію, пошуковий аліас, файли-джерела статей правил 2024 (`rules-uk/batch-06/07/10`, `beyond-srd-uk/batch-04`) і опис «Дикої форми» 2024 у `data/2024/normalized/classes.json`. Опис фічі в базі переписано рівно з файлу на клоні й у проді (звірка: прод відрізнявся лише цими трьома фрагментами); `generate:rules-2024`, `generate:rules-beyond-srd`, `generate:classes`. Лишається 2014: «РЗ (CR)» в описі «Знищення нежиті» (з бази, без файлу-джерела).
...
**Статус:** ✅ закрито раніше, звірено 2026-09-14: суфікса « 2024» у назвах походжень немає.
...
**Статус:** ✅ закрито 2026-09-13 у файлі-джерелі. Розбір SRD у `data/2024/normalized/feats.json` загубив вступний абзац двох рис — Skilled і Ability Score Improvement (перевірено звіркою всіх рис із `data/2024/srd/feats.md`; решта втратила лише службове «You gain the following benefits.»). Обом дописано `plainDescriptionEng` і `description`; сід бере `description` замість `benefits`. Прогнано `seed:2024:test --only feats` і повторно `seed:remove-asi-feat-2024:test --apply` (сід рис повертає ASI). **Прод не засіяно:** у `feats.json` лежать незакомічені правки паралельної сесії (посилання на правила). На листі обрані в рисі навички більше не лягають окремими картками «Атлетика · Риса», а стоять рядком «Обрано: …» в описі риси (`buildFeatFeatureDescription`).
...
**Статус:** ✅ закрито 2026-09-13 (KR31.5). Характеристика — окрема група «Базова характеристика заклинань» (INT/WIS/CHA) у сіді `featMechanics2024.ts`; список більше її не диктує. Прод і клон засіяно з однаковими id 4860–4862, каталог конструктора перегенеровано. Доказ — `tests/actions/magic-initiate-2024.test.ts` (Чарівник + Харизма → MAGIC_INITIATE · CHA) і журнал KR31.5.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.5). Фіча риси дає Misty Step / Invisibility (`BENEFIT_SPELLS_2024` у `featResources2024.ts`). Заклинання 1-го рівня школи гравець обирає окремим кроком майстра підвищення (`LevelUpFeatSpellStep`, правило `feat-spell-choices.ts`), і сервер відхиляє неповний або чужий вибір. Обидва заклинання лягають `pers_spell` з `origin FEAT`, підготовленими й поза лімітом. Джерело на листі — риса з характеристикою свого підвищення. Безкоштовне застосування — наявна фіча-лічильник KR31.3 (Р38). Доказ — `tests/actions/touched-feats-spells-2024.test.ts` і прохід у Chromium (журнал KR31.5).
...
**Статус:** ✅ закрито 2026-09-04 разом із P1-human-fighter-01 ([KR31.1](kr31.1-edition-end-to-end.md)): `MultiStepForm` шле редакцію, риса Skilled 2024 потрапляє у вибірку, три навички лягають у `pers_skill`. Тримають `tests/actions/creation-ruleset.test.ts` і К5 у `tests/rules-2024/acceptance-ten.test.ts` (Noble + Skilled → ARCANA, INSIGHT, RELIGION). Статус дописано 2026-09-13.
...
**Статус:** ✅ закрито для 2024 2026-09-14 (KR31.5, [Р42](../DECISIONS.md#р42)). Порція риси —
«бонус майстерності мінус уже взяті» ритуальні заклинання 1-го рівня. Цю порцію дає крок «Заклинання
риси» при взятті риси, а на рівні, де бонус росте, — окремий крок з полем `featGrowthSpellIds`.
Рядки риси завжди підготовлені й поза лімітом, характеристика — підвищена рисою. Гейти —
`tests/rules/feat-spell-choices.test.ts`, `tests/actions/ritual-caster-spells-2024.test.ts`,
`tests/components/levelup-feat-spell-step.test.tsx`. Лишається «Quick Ritual» (раз на довгий
відпочинок без слота).
...
**Статус:** ✅ закрито для 2024 2026-09-13 (KR31.5, [Р42](../DECISIONS.md#р42)). Після опцій риси йде крок «Заклинання риси»: два замовляння й заклинання 1-го рівня з обраного списку. Він є і в конструкторі (риса від передісторії й від Універсальності Людини, кожна окремо), і в майстрі підвищення. Обране лягає рядками `origin = FEAT`, `isPrepared` і поза лімітом; сервер відхиляє неповний вибір, чужий список і заклинання, яке вже дає клас, вид або друга така сама риса. Риса 2014 списку не питає взагалі — це окрема діра, не закрита. Рису, додану з листа, ця правка не охоплює.
...
**Статус:** ✅ закрито 2026-09-14 у KR31.14. Смуга слайдів стоїть на `bottom-[calc(75px+env(safe-area-inset-bottom,0px))]` — навбар росте з інсетом, смуга тепер теж. Живим iPhone не перевірялося: лист читає базу, а копії на :3100 не піднімали.
...
**Статус:** ✅ закрито 2026-09-14 у KR31.14. Стрілки лише від `md:` (`hidden md:flex`); на телефоні лишаються свайп і смуга знизу.
...
**Статус:** ✅ закрито 2026-09-14 у KR31.14: на телефоні (iPhone 13, Chromium) шапка листа, рядки навичок, дії рядків і фільтри каталогів заклинань, чипи категорій і фільтрів, вкладки листа й конструктора — ≥ 40 px; десктоп без змін (`max-md:` / `md:`). Свідомо поза: перемикачі, кнопка-назва персонажа, посилання в прозі. Деталі — журнал KR31.14.
...
**Статус:** ✅ закрито 2026-09-14 (KR31.5). `src/rules/prepared-spell-limits.ts` рахує
підготовлені окремо на кожен рядок класу. Заклинання йде до класу за бейджем — тим самим, яким його
пише вибір заклинань класу. Лист показує «Підготовлено: X / N» під кожним класом, а спільної стелі
в мультикласі більше немає. Підготовлене без бейджа класу виноситься окремим рядком. Гейти —
`tests/rules/prepared-spell-limits.test.ts` і `tests/components/magic-slide-prepared-limits.test.tsx`.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.10). Блок «Атаки та заклинання» несе рядок «Майстерність зброї: Довгий меч (Виснаження), …» з `pers_weapon_mastery`. `tests/pdf/character-pdf-fidelity.test.ts`.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.10). `src/server/pdf/spellSheetLayout.ts` тримає пари «рядок — кружечок», зняті з координат віджетів (усі 92 зійшлися, зайвих немає); кружечок = `PersSpell.isPrepared`. `tests/pdf/character-pdf-fidelity.test.ts`.
...
**Статус:** ⛔ не робиться — рішення власника 2026-09-13: гроші, як і поточні хіти й кістки хітів, лишаються порожніми під олівець (свідома правка 3962745, січень 2026).
...
**Статус:** ⛔ не робиться — рішення власника 2026-09-13: передісторію й нотатки не друкуємо. Мертвий запис у неіснуючі поля `Backstory`/`Notes` прибрано.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.10). `paginateSpellsByLevel`: рівень, що не вміщується, продовжується на наступній копії листа заклинань (слоти лише на першій). `tests/pdf/character-pdf-fidelity.test.ts`: 14 заклинань 1-го рівня доїжджають усі.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.10). Звичайні слоти — у «К-ть слотів», пактові — підписом «+2 пакт · кор. відп.» у сусідній клітинці. `tests/pdf/character-pdf-fidelity.test.ts`.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.10). `buildPersSpeciesName` (`src/lib/logic/pers-species-name.ts`): 2024 — вид + родовід із груп `SPECIES_LINEAGE_CHOICE_GROUPS`, 2014 — підраса або варіант. На аркуші 2024 анотація FreeText «Раса» замінюється підписом «Вид» (підписи бланка — анотації, не текст сторінки). `tests/pdf/character-pdf-fidelity.test.ts`, `tests/logic/pers-species-name.test.ts`.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.10). Тип дії й лічильники несуть усі чотири носії (KR31.3, 2026-09-08); друк перевірено: у барда 2024 «Бардське натхнення» йде в бонусні дії з числовим лічильником (`tests/pdf/character-pdf-fidelity.test.ts`). Сторінка «Здібності» бере фічі з рядків `pers_feature` — персонаж без них отримує порожню сторінку, як і лист.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.10). Кожна секція збирається до двох разів; якщо не вийшло — на її місці сторінка «Секцію «…» не вдалося сформувати. Спробуйте завантажити PDF ще раз.» (рішення власника). `PDF_STRICT_SECTIONS` більше не читається. `tests/pdf/character-pdf-fidelity.test.ts` з підміненим `generateFeaturesPdfBytes`.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.10). Зброя понад три рядки шаблона друкується в блоці «Атаки та заклинання» під «Ще зброя:» з бонусом і шкодою. `tests/pdf/character-pdf-fidelity.test.ts`.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.10). `loadPrintableSpells` віддає назву книги з `sourceTranslations`. `tests/pdf/character-pdf-fidelity.test.ts`.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.10). Усі сеттери текстових полів PDF проходять `stripGlossaryMarkers`. `tests/pdf/character-pdf-fidelity.test.ts`.
...
**Статус:** 🔴 рішення власника 2026-09-13: «Додати рису» на листі — повне набуття (ті самі вибори й надання, що в левелапі). Відкладено до закриття KR31.5: застосування надань треба виносити з `levelup-persistence.ts`, який та робота зараз править.
...
**Статус:** ✅ закрито 2026-09-13 у KR31.8 як дубль `L16-ruleset-isolation-03`: каталог спорядження листа береться за редакцією персонажа.
...
**Статус:** ✅ закрито 2026-09-13. Лист і PDF перейшли на швидкість виду ще в KR31.6 (`calculateWalkingSpeed`). Лишалася модалка правки «ШВИДКІСТЬ», де база була `30`: тепер це `calculateFinalSpeed` мінус ручна поправка, як у КБ та ініціативи. `tests/components/modify-stat-speed-base.test.tsx` на старому коді червонів `Базове: 30→Фінальне: 35`.
...
**Статус:** ⛔ не робиться — рішення власника 2026-09-13: станів, виснаження й концентрації на листі не буде в принципі.
...
**Статус:** ✅ закрито 2026-09-13 у KR31.8 як дубль `L16-ruleset-isolation-04`: персонаж 2024 додає предмети з `/2024/magic-items`.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.13) — ручний лічильник за рішенням власника. `pers_magic_item.charges_max/charges_current` (SQL застосовано до робочої бази й `spells_test`, `db:pull`); правило `src/rules/magic-item-charges.ts`; дії `setMagicItemChargesMax`/`stepMagicItemCharges`; `MagicItemChargesControl` на картці предмета («+ заряди», −/+, клік по числу міняє максимум); копії й знімки переносять заряди. Автовідновлення на світанку немає свідомо.
...
**Статус:** ⛔ не робиться, рішення власника 2026-09-14: вага й навантаження не потрібні.
...
**Статус:** ⛔ не робиться — рішення власника 2026-09-13: станів і виснаження на листі не буде в принципі.
...
**Статус:** ✅ закрито 2026-09-14 рішенням власника: помилку виправляє історія персонажа (знімки рівнів, KR31.7) — окремого респеку не буде.
...
**Статус:** ↗ передано 2026-09-14: над своїми фічами працює інша сесія (рішення власника), у KR31.13 не робиться.
...
**Статус:** ⛔ не робиться, рішення власника 2026-09-14: володіння й мови лишаються текстом.
...
**Статус:** ↗ передано 2026-09-14: портретом і зовнішністю займається інша сесія (рішення власника).
...
**Статус:** ⛔ не робиться, рішення власника 2026-09-14: інвентар лишається текстом.
...
**Статус:** 🟡 2026-09-13 (KR31.13). Тип шкоди й дальність доведені до гравця: поля в `WeaponCustomizeModal` («Як у книзі» чи свій тип; дальність і макс. дальність), `updateWeapon` їх приймає, картка зброї показує тип і дальність через `findWeaponDamageType`/`findWeaponRange` у `bonus-calculator.ts`, змінена зброя не зливається в групу однакових. Тести: `tests/db/weapon-customize-overrides.test.ts` (на старому коді `[null,null,null]`), три юніт-перевірки в `bonus-calculator.test.ts`. `pers_skill.custom_modifier` власник вирішив прибрати: у робочій базі 0 непорожніх із 50 337; SQL — `db/changes/2026-09-13-kr31.13-drop-pers-skill-custom-modifier.sql`; власник: застосувати після деплою, до того не чіпати.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.6) рішенням власника «похідний список + текст». `collectDerivedProficiencies` (`src/rules/derived-proficiencies.ts`) зводить фіксовані надання обладунків, зброї, інструментів і мов; `calculatePersProficiencies` (`src/lib/logic/pers-proficiencies.ts`) бере вид, підвид, вибори виду, передісторію, основний клас, пакет мультикласу, підкласи, риси й активні фічі. Лист показував блок «З джерел персонажа» над полями «Володіння» й «Мови», PDF — перед ручним текстом без дублікатів рядків. **2026-09-13, власник: блок прибрано, відсутні надання дописуються в сам текст** (`appendMissingProficiencies`, див. [KR31.6](kr31.6-sheet-derived.md)). Вибори (мова чи інструмент на вибір) живуть у тексті, як і раніше; текст наявних персонажів не чиститься. `tests/db/pers-proficiencies.test.ts`: дворф Воїн 3 / Пройдисвіт 2 2014; без пакета мультикласу — `[] ≠ [THIEVES_TOOLS]`.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.5). Корінь — втрата редакції при створенні (`P2-elf-wizard-01`), виправлений 2026-09-04: схема більше не підставляє `RULES_2014`, і редакція береться з класу. Новий кейс у `tests/actions/creation-ruleset.test.ts` будує високого ельфа-чарівника 2024 тим самим запитом, що й форма (без поля `ruleset`, з `raceChoiceSelections` родоводу), і вимагає Prestidigitation у `pers_spell`. Доведено червоним: з поверненим дефолтом `RULES_2014` у схемі — `expected [] to include 'Prestidigitation'`. Живий Chromium цього разу не ганявся.
...
**Статус:** ✅ закрито 2026-09-13 (KR31.5, [Р43](../DECISIONS.md#р43)). Конструктор 2024 має крок «Заклинання» після класових виборів: замовляння й підготовлені за таблицею класу на 1-му рівні, у чарівника ще 6 заклинань до книги, з яких він готує; Тауматург і Маг дають +1 замовляння. Кандидати — список класу без того, що клас дає сам. Сервер відмовляє без повного вибору, чужого списку й заклинання, яке вже дає вид; обране лягає `pers_spell` (`CLASS`, бейдж класу, книга поза підготовкою — `isPrepared = false`). Тести: `tests/rules/creation-spells-2024.test.ts` (9), `tests/actions/creation-spells-2024.test.ts` (6), `tests/components/creation-spells-form.test.tsx` (3), кожен доведено зламом. Живий Chromium на копії зі `spells_test`: клірик-тауматург обирає 4 + 4, «Далі» відкривається лише з повним вибором, «Створити» дає персонажа RULES_2024 з вісьмома заклинаннями, лист їх показує.
...
**Статус:** ✅ закрито 2026-09-14 (KR31.5, [Р42](../DECISIONS.md#р42), [Р43](../DECISIONS.md#р43)).
Бард 2024 з 10-го рівня обирає нові підготовлені заклинання в кроці «Заклинання» (конструктор і
підвищення) зі списків барда, клірика, друїда й чарівника. Замовляння лишаються бардівськими, обране
лягає з бейджем «Бард», тож рахується бардівським. Фільтр вибору приймає кілька списків. Гейти —
`tests/rules/class-spell-choices-2024.test.ts` і `tests/actions/magical-secrets-2024.test.ts`.
Лишається заміна підготовленого (у продукті її немає для жодного класу) і діалог «Додати» на листі.
...
**Статус:** ✅ закрито для 2024 2026-09-14 (KR31.5, [Р43](../DECISIONS.md#р43)). З 2026-09-13 для 2024 майстер має крок «Заклинання»: скільки дає таблиця класу на новому рівні мінус уже наявне з бейджем класу — нові замовляння, підготовлені, +2 до книги чарівника; мультиклас бере список і числа нового класу, Лицар-Чаклун і Містичний спритник — список чарівника. Сервер без повного вибору рівня не підвищує. Тести: `tests/actions/levelup-spells-2024.test.ts` (5), `tests/rules/class-spell-choices-2024.test.ts` (+7), `tests/components/class-spell-choice-step.test.tsx` (1), кожен доведено зламом; живий Chromium пройшов чарівника 1 → 2.

2026-09-14 — заміна одного заклинання на рівні, закрито для 2024 у KR31.5. За SRD 2024 замовляння на новому рівні міняють бард, клірик, друїд, чародій і чорнокнижник; підготовлене — бард, чародій і чорнокнижник (у чарівника, клірика, друїда, паладина й слідопита підготовлені міняються на тривалому відпочинку, і лист це вже дозволяє). Крок «Заклинання» має необовʼязкові секції «Замінити замовляння» / «Замінити підготовлене заклинання» і зʼявляється навіть на рівні без нових заклинань, якщо є що замінити; «Далі» закрите, поки заміну почато, але не доведено. Сервер дає прибрати лише власне заклинання з бейджем класу (не від риси чи виду), нове — з того самого фільтра класу й не обране вже на цьому рівні; старий рядок видаляється, новий пишеться підготовленим. Правило — `src/rules/class-spell-swaps-2024.ts`. Тести: `tests/rules/class-spell-swaps-2024.test.ts` (3), `tests/components/class-spell-choice-step.test.tsx` (+1), `tests/actions/levelup-spells-2024.test.ts` (+2), кожен доведено зламом. Живий Chromium (iPhone 13, копія зі `spells_test`): чародій 1 → 2, Захист від зброї → Штукарство, Палючі долоні → Щит плюс два нові підготовлені; на половині заміни «Далі» вимкнене; у базі 4 замовляння й 4 підготовлені, прибраних рядків немає. 2014 у цей KR не входить.

2026-09-16 — 2014 зроблено в [KR31.18](kr31.18-spells-2014-levelup.md) за [Р44](../DECISIONS.md#р44): прибавка рівня за таблицею PHB обовʼязкова, решта до таблиці — за бажанням, бард, чародій, чорнокнижник і слідопит можуть замінити одне відоме заклинання. Живий Chromium пройшов барда 5 → 6. 2026-09-18 — Потойбічний лицар і Містичний спритник (список чарівника, школи підкласу, «будь-якої школи» на 3/8/14/20) і розширені списки покровителів (зведені з 5etools, засіяні в прод), рід Джина як вибір підкласу — там само.
...
**Статус:** ✅ закрито 2026-09-14 (KR31.5). Звірка йде в один бік: бейдж виводить заклинання з
ліміту, лише коли містить цілу назву виду, підвиду, підкласу або слово «підклас»/«раса». Уривки
«Клас», «Рас», «Ліс», «Домен» більше не спрацьовують. Прив'язку до `origin` не робили: бейджі вручну
стоять у рядках 2014 на проді, і перехід змінив би лічильники всім. Гейт —
`tests/logic/spell-prepared-exclusions.test.ts`.
...
**Статус:** ⛔ не робиться — рішення власника 2026-09-13 у [KR31.13](kr31.13-sheet-configurability.md)
(`L09-sheet-derived-09`): «станів, виснаження й концентрації на листі не буде в принципі».
...
**Статус:** ✅ закрито 2026-09-14 (KR31.5). У рядку заклинання 1-го рівня й вище є кнопка «Накласти»
(чарівна паличка). Меню пропонує слоти рівня заклинання й вищі та слот пакту, якщо його рівень не нижчий;
порожні вимкнені. Обраний слот списується тим самим шляхом, що й клік по слоту (з офлайн-чергою).
Правило — `src/rules/spell-casting-slots.ts`, меню — `characterSheet/shared/CastSpellMenu.tsx`. Гейти —
`tests/rules/spell-casting-slots.test.ts`, `tests/components/magic-slide-cast-spell.test.tsx`; живий
Chromium (iPhone 13). Текст «На вищих рівнях» окремо не показується: у базі він частина опису.
...
**Статус:** ✅ закрито 2026-09-14 у KR31.15. `findFormFeature` шукав рівно `Wild Shape`, а фіча 2024 — `Druid: Wild Shape (2024)`; тепер тип «звір» знає обидві назви. Пул і числа (2/3/4 за рівнем) фіча вже мала з KR31.3, тож лічильник на картці й списання при вході у форму запрацювали без даних. Тести: `tests/rules/wildshape-uses.test.ts` і `tests/actions/wildshape-actions.test.ts` («2 рівень — 2 / 2, вхід у вовка знімає одне»; до правки `findWildshapeUses` давав `null`).
...
**Статус:** ✅ закрито раніше в KR31.3, звірено 2026-09-14: `Druid: Wild Shape (2024)` у `FEATURES_REGAINING_ONE_USE_ON_SHORT_REST` (`src/rules/resource-pools.ts`), короткий відпочинок пулу йде через `findUsesAfterShortRest`.
...
**Статус:** ✅ закрито раніше в KR31.7, звірено 2026-09-14: `clonePersWithRelations` копіює `wildshapes` (`copyResourcePoolsAndWildshapes`).
...
**Статус:** ✅ закрито 2026-09-14 у KR31.15 як підказка (Р26): `findMissingFreeBasicSpaces` і таблиці `BASIC_FACILITY_COSTS` / `BASIC_FACILITY_ENLARGEMENT_COSTS` у `src/rules/bastions.ts` (числа звірено з `book-xdmg.json`); сторінка бастіону показує, якого з двох безкоштовних (тісне, просторе) ще бракує, і вартість наступних базових та збільшення. Золото й дні не списуються.
...
**Статус:** ✅ закрито 2026-09-14 у KR31.15: `saveFacilityState` приймає `space`, звіряє з каталогом (`readResizedSpace` → `readFacilitySpace`) і пише розмір тим самим оновленням, що наказ, захисників, найманців і нотатки; у формі стану — селект «Розмір» для приміщень із більш ніж одним розміром. Тест `tests/actions/bastion-facilities.test.ts`: спальня стає просторою з нотатками, кузні розлогий розмір не дається.
...
**Статус:** ✅ закрито 2026-09-14 у KR31.15. Крок «Перевірка змін» майстра підвищення показує `BastionLevelUpNote`: скільки нових спеціальних приміщень дає рівень і право замінити одне (`describeBastionLevelUp`); персонажу без бастіону — лише на переході до 5-го. Заміна — дія `replaceFacility` і селект «Замінити» в рядку спеціального приміщення: рядок той самий, захисники, найманці й нотатки лишаються, розмір і наказ — якщо новий каталог їх дозволяє (`findReplacementState`); на базове не міняє. Скільки замін було за рівень, застосунок не рахує (Р26).
...
**Статус:** ✅ закрито раніше в KR31.7, звірено 2026-09-14: `clonePersWithRelations` копіює бастіон із приміщеннями й журналом (`copyBastion`).
...
**Статус:** ✅ закрито 2026-09-14 у KR31.15: глава 8 DMG 2024 — стаття `/2024/rules/adventuring#bastions` (`beyond-bastions`, 9 підрозділів, ≈4 470 слів дослівно за Р27; статблоки приміщень виключено — вони в каталозі). Розкладач `scripts/5etools/xdmg-chapters.ts`, партія `data/2024/beyond-srd-uk/batch-09.json`. Посилання «Правила бастіонів» стоїть на сторінці бастіону й у каталозі `/2024/bastions`. Терміни, обрані перекладачем, винесено власнику (журнал KR31.15).
...
**Статус:** ✅ закрито 2026-09-14 у KR31.15: пікер каталогу показує «Вже в бастіоні — спеціальне береться один раз» (`isSpecialFacilityAlreadyBuilt`), додавання не блокує (Р26); базових це не стосується — книга дозволяє їх повторювати.
...
**Статус:** ✅ закрито 2026-09-14 рішенням власника «так, лише читання». Поширений лист показує картку «Бастіон» на слайді Рис; вона відкриває `SharedBastionDialog` — приміщення, розмір, наказ, захисники, найманці й Утримання, без нотаток і журналу. Доступ дає сам токен (`loadSharedBastion` → `findSharedBastionView`, обидва види токена). Знімок і PDF бастіону свідомо не несуть.
...
**Статус:** ✅ закрито 2026-09-14 рішенням власника (окремий перемикач). DDL `db/changes/2026-09-14-kr31.15-bastion-maintain.sql` — `pers_bastion.is_maintaining`, застосовано на клоні й власником у проді, `db:pull` (поле `isMaintaining`). Перемикач «Утримання» над приміщеннями (`BastionMaintainToggle`, дія `saveBastionMaintaining`); приміщенням наказ більше не пропонується (`findAllowedOrderCodes`, `FACILITY_ORDER_CODES`), сервер його для приміщення відхиляє; накази приміщень не стираються — `describeMaintainConflict` попереджає (Р26). Копія персонажа переносить прапорець.
...
**Статус:** ✅ закрито 2026-09-14 рішенням власника «приглушена картка». `findBastionAccess` показує картку всім персонажам 2024 (`isEntryCardShown`), а до 5-го рівня без бастіону — приглушеною з написом «за правилами з 5-го рівня» (`isEntryCardMuted`).
...
**Статус:** ✅ закрито 2026-09-14 у KR31.15: рядок приміщення пише «Базове · Тісне» або «Спеціальне · рівень 5+ · Просторе» (`BastionFacilityKindLabel`).
...
**Статус:** ↗ передано 2026-09-14: власним контентом займається інша сесія (рішення власника).
...
**Статус:** ↗ передано 2026-09-14: 3D-кубиками для навичок, ряткидків і заклинань займається інша сесія (рішення власника).
...
**Статус:** ⛔ не робиться, рішення власника 2026-09-14: компаньйонів як сутностей не буде.
...
**Статус:** ↗ передано 2026-09-14: портретом займається інша сесія (рішення власника).
...
**Статус:** ✅ не відтворено 2026-09-14 ні анонімно, ні з сесією (підроблена cookie на копії :3100 зі `spells_test`): `/char/create`, `?ruleset=RULES_2024`, `/char/<id>/levelup` для 2014 і 2024, також після примусової перекомпіляції `providers.tsx` і `MultiStepForm.tsx` — 200, без `pageerror` і без відкату на клієнтський рендер. Усі шість викликів `useSession` лежать під `<Providers>` кореневого layout, `next-auth` один (5.0.0-beta.30). Найімовірніше, аудит зловив разове розходження модулів під час перекомпіляції дерева, яке правили паралельні сесії; мертвий `useSession()` в `EditionSwitcher` прибрано ще в KR31.11.
...
**Статус:** ✅ закрито 2026-09-14 у KR31.14. Для 2024 модалка показує «Характеристики на вибір», «Риса походження» (назву дає крок через `feats`) і «Спорядження: пакунок або 50 зм»; порожні «Мови» й «Особливість» — лише для 2014.
...
**Статус:** ✅ закрито 2026-09-13. «Куди дивитись» хибне: у даних цього рядка немає, його вписував сам `FeatChoiceOptionsForm.tsx`, коли зливав дубльовані групи навичок Skilled в одну. Заголовок групи тепер «Навички»; ключ `Skilled Options` лишився, бо під ним лежать вибори в чернетках гравців, а сервер ключів не читає (`Object.values`). Інструментів у групі досі немає — це [P1-human-fighter-11](#p1-human-fighter-11--риса-умілець-пропонує-лише-18-навичок--інструментів-дозволених-книгою-у-списку-немає).
...
**Статус:** ✅ закрито 2026-09-14: «Вид» — як `P7-mobile-ux-06`; Таші — як `P6-class-sweep-level1-11` (рішення власника: сховати в 2024).
...
**Статус:** ✅ закрито 2026-09-14 у KR31.14. Підсумок на кроці «Імʼя» бере підпис із `describeEquipmentRow` — того самого правила, що й крок «Спорядження», яке читає поле `item`; окрема копія з `as any` прибрана.
...
**Статус:** ✅ закрито раніше, звірено 2026-09-14: `collectOriginLanguages` додає «Загальну» до відомих мов 2024, `listChoosableLanguages` її відсіює; тести `tests/rules/origin-languages.test.ts`.
...
**Статус:** ✅ закрито 2026-09-14 рішенням власника: «Правила Таші» в 2024 не діють. `SkillsForm` ховає перемикач і скидає `isTasha` зі старої чернетки для класу 2024; `character-creation.ts` не бере `tashaChoices` для персонажа 2024. Тести: `tests/components/skills-step-gate.test.tsx`, `tests/actions/tasha-skills-2024.test.ts`.
...
**Статус:** ✅ закрито 2026-09-14 у KR31.14. `resolveCreationSteps` приймає `is2024`: кроки «Вид» / «Опції виду», заголовок `RaceChoiceOptionsForm` — за редакцією виду. Перевірено в Chromium 390×844 на `/2024/char`.
...
**Статус:** ✅ закрито 2026-09-14 у KR31.14. Спільне `buildSelectableCardProps` (`selectable-card-props.ts`) дає картці `role=radio|checkbox`, `tabIndex`, `aria-checked`, Enter/Пробіл і не обирає опцію, коли тап влучив у `[data-stop-card-click]` (посилання на заклинання й терміни). Підключене в `RaceChoiceOptionsForm` і `ClassChoiceOptionGroups`. Chromium: тап по «Отруйні бризки» відкриває модалку й лишає `aria-checked=false`, тап по назві картки обирає.
...
**Статус:** ✅ закрито 2026-09-14 у KR31.14. «Попередній слайд» / «Наступний слайд».
...
**Статус:** ✅ закрито раніше, звірено 2026-09-14: `sortAvailableFirst` опускає недоступні опції донизу, `getCardClassName` їх приглушує.
