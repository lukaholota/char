...
describe("таблиця Звіриних форм 2014", () => {
  ...
  /// Зловили тестом: у каталозі повно звірів із ПН 0, тому «межа нуль» пропускала б їх усіх
  /// персонажу, який Дикої форми не має взагалі.
  it("без Дикої форми не пропонується жодного звіра, навіть із ПН 0", () => {
  ...
  it("2 рівень — ПН 1/4, без польоту й плавання", () => {
  ...
  it("4 рівень — ПН 1/2, плавання дозволене, політ ні", () => {
  ...
  it("8 рівень — ПН 1 і жодних обмежень руху", () => {
...
describe("коло Місяця", () => {
  it("з 2 рівня дає ПН 1 замість 1/4", () => {
  ...
  /// і плавання відкриваються за загальною таблицею, а не разом із ПН.
...
describe("придатність конкретної форми", () => {
  ...
  it("не-звір недоступний навіть за ПН", () => {
    expect(blockingKinds(SUCCUBUS, druid(20, true))).toEqual(["notABeast"]);
  });

  /// Девʼять звірів каталогу — статблоки прикликання без ПН узагалі. Казати про них «показник
  /// небезпеки завеликий» неправда: його немає.
  it("звір без показника небезпеки відхиляється своєю причиною, а не «завеликим ПН»", () => {
    expect(blockingKinds(BESTIAL_SPIRIT, druid(20, true))).toEqual(["noChallengeRating"]);
  });

  /// Дві причини одразу, а не перша-ліпша: на 2 рівні акулу ріже і ПН, і плавання, і гравець
  /// має бачити обидві — інакше він підніме рівень і зустріне ту саму відмову з іншим текстом.
  it("акула недоступна на 2 рівні через ПН і плавання, а на 4 доступна", () => {
  ...
  it("плавання ріже форму, яка проходить за ПН", () => {
  ...
  /// не бачить орла — і причина орла саме політ, а не ПН, який коло вже підняло до 1.
  it("друїд Місяця 2 рівня бачить ведмедя й не бачить орла — через політ, не через ПН", () => {
    expect(findWildshapeEligibility(BROWN_BEAR, druid(2, true)).eligible).toBe(true);
    expect(blockingKinds(GIANT_EAGLE, druid(2, true))).toEqual(["flySpeedLocked"]);
  });

  it("причин може бути дві одразу — ПН завеликий і політ зарано", () => {
...
describe("причина несе поріг і текст", () => {
  ...
  it("завеликий ПН називає обидва числа — істоти й межі", () => {
...
describe("рядок обмежень для картки", () => {
  ...
  it("називає ПН, обидва пороги руху й те, що лазіння не обмежене", () => {
...
describe("таблиця Звіриних форм 2024", () => {
  ...
  it("2 рівень — ПН 1/4, без польоту, але плавання вже дозволене", () => {
  ...
  it("4 рівень — ПН 1/2, політ і далі закритий", () => {
  ...
  it("8 рівень — ПН 1 і політ відкрито", () => {
...
describe("Коло місяця 2024", () => {
  /// Підклас у 2024 починається з 3 рівня, і ПН одразу дорівнює третині рівня друїда — без
  /// сходинки «ПН 1», яку 2014 тримає до 6 рівня.
  it("з 3 рівня ПН дорівнює третині рівня друїда", () => {
  ...
  it("до 3 рівня коло нічого не піднімає — ПН лишається табличний", () => {
...
/// Статблоки прикликання — «Дух звіра», «Первісний супутник» і подібні — типом звірі, але ПН у
...
describe("статблоки прикликання в каталозі", () => {
  ...
  it("девʼять звірів без ПН відхиляються причиною «немає показника небезпеки»", () => {
